...
event_volume_summary <- event_volume_summary %>% mutate(Class = case_when(
...
names(myColors4) <- levels(event_volume_summary$Class)
colorScale <- scale_color_manual(name = "Class",values = myColors4)


volume_plot <- ggplot(subset(event_volume_summary,Class != "unanalyzed" & Class != "WT"), aes(x=reorder(Event,mean_Volume),y=mean_Volume, color = Class))+
  geom_point()+
