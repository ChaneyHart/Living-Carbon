...
weather_subset <- subset(LC_2023_weather, select =c(Datetime,air_temp, VPD))
...
weather_hourly <- subset(weather_hourly, select = c("Hourly","air_temp_mean","VPD_mean","air_temp_max","VPD_max"))

colnames(weather_hourly) <- c("Datetime","air_temp_mean","VPD_mean","air_temp_max","VPD_max")
