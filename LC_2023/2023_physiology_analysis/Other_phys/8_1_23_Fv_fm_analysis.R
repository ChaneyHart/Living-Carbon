...
Fv_fm_dat_8_1 <- read.csv("LC_2023/2023_physiology_analysis/Fv_fm_8_1_23/2023-08-01/Fv_fm_8_1_filled.csv")
Fv_fm_dat_8_2 <- read.csv("LC_2023/2023_physiology_analysis/Fv_fm_8_2_23/2023-08-02/8_2_23_fv_fm_filled.csv")
...
Fv_fm_summ <- Fv_fm_summ%>% mutate(Class = case_when(
...
Fv_fm_event <- Fv_fm_event%>% mutate(Class = case_when(
  event_short == "5A" | event_short == "5C" | event_short == "4A" ~ "elite",
  event_short == "13-15E" | event_short == "2H" ~ "poor",
  event_short == "16-20" | event_short == "8-9D" | event_short == "CT3" ~ "control"))

ggplot(Fv_fm_event, aes(y = Fvfm, x =event_short, color = Class))+
  geom_point()+
  scale_color_manual(values = my_colors)+
  geom_errorbar(ymin = Fvfm-Fvfm_se, ymax = Fvfm + Fvfm_se)


Fv_fm_plot <- ggplot(Fv_fm_event, aes(x = event_short, y=Fvfm, color = Class))+
  geom_point()+
  geom_errorbar(aes(ymin = Fvfm-Fvfm_se, ymax = Fvfm + Fvfm_se))+
  scale_color_manual(values = my_colors)+
...
ggsave(filename = "July_Fv_fm.png", plot=Fv_fm_plot, dpi =300)
