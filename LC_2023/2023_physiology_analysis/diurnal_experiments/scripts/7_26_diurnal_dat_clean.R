...
dat_7_26_6800$Ci <- as.numeric(dat_7_26_6800$Ci)
...
July_gsw <- ggplot(subset(July_summary_class,treatment == "drought"), aes(x = Timepoint, group=Class,color = Class, shape = treatment))+
...
July_ETR <- ggplot(subset(July_summary_class,treatment == "drought"), aes(x = Timepoint, group=Class,color = Class, shape = treatment))+
  geom_point(aes(y = ETR_6800,alpha=treatment), size = 2)+
  geom_line(aes(y = ETR_6800, linetype = treatment,alpha=treatment))+
  scale_alpha_manual(values=c(1,0.5))+
  geom_errorbar(aes(ymin = ETR_6800-ETR_6800_se, ymax = ETR_6800+ETR_6800_se,linetype = treatment,alpha=treatment), width = 0.2)+
  scale_color_manual(values = my_colors)+
...
July_PhiPS2 <- ggplot(subset(July_summary_class,treatment == "drought"), aes(x = Timepoint, group=Class,color = Class, shape= treatment))+
...
July_Tleaf <- ggplot(subset(July_summary_class,treatment == "drought"), aes(x = Timepoint, group=Class,color = Class, shape = treatment))+
...
July_diurnal_phys_plot <- grid.arrange(July_assimilation, July_ETR, July_gsw,July_Tleaf, nrow=3)
...
July_fluor_set$Theta_e <- (4*(July_fluor_set$PhiPS2 - 0.026947))/7.567127
July_fluor_set$Jt <- July_fluor_set$Theta_e*July_fluor_set$PAR
...
July_fluor_set$R_6800 <- (-1*July_fluor_set$Rd*2.2)^((July_fluor_set$Tleaf - July_fluor_set$Resp_temp)/10)

July_fluor_set$Theta_e_6800 <- (4*(July_fluor_set$PhiPS2_6800 - 0.026947))/7.567127
#values derived from low O2 conditions
July_fluor_set$Jt_6800 <- July_fluor_set$Theta_e_6800*July_fluor_set$Qin
July_fluor_set$Jo_6800 <- (2/3)*(July_fluor_set$Jt_6800 - (4*(July_fluor_set$A + July_fluor_set$Rd)))
July_fluor_set$Rl_6800 <- (1/12)*(July_fluor_set$Jt_6800 - (4*(July_fluor_set$A + July_fluor_set$Rd)))
July_fluor_set$Jc_6800 <- (1/3)*(July_fluor_set$Jt_6800 + (8*(July_fluor_set$A + July_fluor_set$Rd)))
July_fluor_set$Jo_ratio_6800 <- July_fluor_set$Jo_6800/July_fluor_set$Jt_6800
